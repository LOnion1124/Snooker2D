...
\textbf{关键约束}：View 层不链接 ViewModel/Model（CMake 编译期强制），ViewModel 不链接 View。所有跨层信号槽绑定集中在 App.cpp（13 条 connect），配合 ViewModel 构造中的 9 条通知绑定，共 22 条跨层连接。
...
\vspace{0.3cm}
\textcolor{gray}{\small 左侧：白球可视化击打点选择器（点击/拖拽红点）；右侧：加塞数值显示 + 方向描述 + 重置按钮}
\vspace{3cm}
\end{minipage}
}
\caption{加塞控制面板}
...
\item \textbf{引入 CI/CD 演示}：本项目已接入 GitHub Actions，每次 push 自动触发 MSVC+Qt 编译、10 套测试运行（221 条断言）和架构护栏检查。建议课程将此配置模板化，帮助学生快速集成持续集成。
