...
Snooker2D 是一个基于 C++17 和 Qt 6 的 2D 斯诺克台球游戏，采用 MVVM 架构，由三名开发者协作完成。项目历经三轮架构重构，从最初的 GameUiBus 中间件方案逐步收敛到 App 层集中绑定的最终形态。相比中期节点，结题阶段新增了加塞（English/Spin）物理系统、中英双语界面、库边预瞄辅助线、GameControlPanel 设置面板，以及覆盖全部五层的单元测试体系。
...
\hline
\textbf{层} & \textbf{职责} & \textbf{编译依赖} & \textbf{源文件数} \\
\hline
Common     & 全局类型、常量、工具函数、DTO       & 无                      & 3 \\
Model      & 游戏状态机、物理引擎、规则判定     & Common + Qt::Core       & 6 \\
ViewModel  & 数据翻译、状态推送、命令处理       & Model + Common + Qt::Core & 1 \\
View       & 界面渲染、用户输入                 & Common + Qt::Widgets    & 8 \\
App        & 对象装配、信号槽绑定               & 所有层                   & 2 \\
\hline
\end{tabular}
\caption{五层架构概览}
\end{table}

\textbf{关键约束}：View 层不链接 ViewModel/Model（CMake 编译期强制），ViewModel 不链接 View。所有跨层信号槽绑定集中在 App.cpp（14 条 connect）。
...
命令绑定 & View → ViewModel & App.cpp & 7 条 \\
...
\vspace{4cm}
\centering
\textcolor{gray}{\large [截图1：游戏主窗口完整界面]}
\vspace{0.3cm}

\textcolor{gray}{\small 左侧：GameView 球桌渲染 + CueControl 力度/角度双滑块}
\textcolor{gray}{\small 右侧：ScoreBoard 计分板 + GameInfoPanel 游戏信息 + EnglishControlPanel 加塞面板 + GameControlPanel 设置面板}
\vspace{4cm}
\end{minipage}
}
...
\vspace{4cm}
\centering
\textcolor{gray}{\large [截图2：物理碰撞效果 — 击球后多球碰撞散开]}
\vspace{0.3cm}

\textcolor{gray}{\small 展示球-球弹性碰撞（恢复系数 0.88）、球-库边反弹（恢复系数 0.77）}
\textcolor{gray}{\small 白球加塞击球：显示击打点标记（白球上的圆点）和库边预瞄辅助线}
\vspace{4cm}
\end{minipage}
}
\caption{物理碰撞与加塞效果}
...
\vspace{4cm}
\centering
\textcolor{gray}{\large [截图3：D 区白球放置模式]}
\vspace{0.3cm}

\textcolor{gray}{\small 白球落袋后进入放置模式：D 区半透明弧线引导，鼠标点击放置}
\textcolor{gray}{\small GameInfoPanel 显示"请将白球放置在 D 区内"提示}
\vspace{4cm}
\end{minipage}
}
\caption{D 区白球复位}
...
\vspace{3cm}
\centering
\textcolor{gray}{\large [截图4：加塞控制面板 — EnglishControlPanel 击打点选择器]}
\vspace{0.3cm}
\textcolor{gray}{\small 左侧：击打点 3×3 网格选择器；右侧：上/下/左/右塞实时数值显示 + 文字描述 + 重置按钮}
\vspace{3cm}
\end{minipage}
}
\caption{加塞控制面板}
...
\vspace{3cm}
\centering
\textcolor{gray}{\large [截图5：中英双语切换 — GameControlPanel 设置面板]}
\vspace{0.3cm}
\textcolor{gray}{\small 设置按钮打开语言/瞄准线选项对话框，支持中文/English 即时切换}
\vspace{3cm}
\end{minipage}
}
\caption{设置面板与双语切换}
...
\vspace{3cm}
\centering
\textcolor{gray}{\large [截图6：终端测试运行结果]}
\vspace{0.3cm}
\textcolor{gray}{\small 展示 10 个测试可执行文件全部通过（test\_mathutils, test\_physics, test\_ball, test\_player,}
\textcolor{gray}{\small test\_rules, test\_gamestate, test\_viewmodel, test\_englishcontrolpanel, test\_gamecontrolpanel, test\_gameview）}
\vspace{3cm}
\end{minipage}
}
\caption{单元测试运行结果}
...
\item \textbf{加塞 UI 面板}：EnglishControlPanel 击打点 3×3 网格选择器，实时显示 englishX/englishY 数值与文字描述
...
\textbf{EnglishControlPanel 加塞面板}：实现 EnglishBallSelector 自定义 Widget — 以 3×3 网格展示白球击打点，中心为正中击球(0,0)，上/下/左/右分别对应 Follow/Draw/Left/Right。每个格子用椭圆填充表示击打区域，当前选中的格子高亮显示。面板底部显示 "上塞 50\%, 右塞 30\%" 形式的文字描述和重置按钮。englishChanged 信号携带 (englishX, englishY) 双精度值发射到 ViewModel。
...
MissedAll（空杆）、WrongBallFirst（先击错球）、WhitePocketed（白球落袋）、NoBallHitCushion（无球碰库）、ColorPocketed（红球阶段彩球意外落袋）、BallOffTable（球飞出台面）、PushShot（推杆）。
...
\subsection{开发者 C（井淳）— Common + App 层 + 文档}

负责公共类型与工具、ViewState DTO 设计、跨层信号槽装配、构建配置、测试体系搭建和项目管理。结题阶段新增工作：
...
\textbf{App 层信号槽装配}：连接数从 10 条增至 14 条。新增 4 条连接：
...
\hline
test\_mathutils       & 62 & 向量运算、几何函数、tangent、类型辅助、常量校验 \\
test\_physics         & 20 & 库边越界反弹、侧旋改变反弹角度、Follow/Draw 碰撞后走位 \\
test\_ball            & 25 & 构造、分球值、位置/速度/角速度/侧旋设置、落袋信号 \\
test\_player          & 15 & 命名、计分、单杆累计、break 重置、信号发射 \\
test\_rules           & 22 & 目标球判定、罚分计算、合法目标检测、多种犯规检测 \\
test\_gamestate       & 18 & 开局流程、D 区放置验证、performShot 门控、玩家访问 \\
test\_viewmodel       & 6  & start 发射全信号、角度归一化、力度/加塞钳位、去重、重启 \\
test\_englishcontrolpanel & 3 & 构造、重置按钮信号、applyCueState 不崩溃 \\
test\_gamecontrolpanel    & 2 & 构造、重启按钮信号 \\
test\_gameview            & 3 & 构造、applyTableState、cancelShotAnimation 不崩溃 \\
\hline
\textbf{合计}         & \textbf{176+} & \\
\hline
\end{tabular}
...
开发者 C（井淳） & 约 18 & Common / App / 文档 & Common 扩展、DTO 设计、App 装配、测试体系、架构护栏、文档 \\
\hline
\end{tabular}
\caption{团队提交统计}
\end{table}
...
\vspace{3cm}
\centering
\textcolor{gray}{\large [截图7：Git 提交历史（结题阶段）]}
\vspace{0.3cm}
\textcolor{gray}{\small 展示 2026-07-11 至 2026-07-18 期间的提交记录，}
\textcolor{gray}{\small 包含加塞功能开发、测试扩展、架构护栏、文档更新等提交}
\vspace{3cm}
\end{minipage}
}
\caption{Git 提交历史}
...
\hline
单元测试编写 & 生成测试框架，人工调整数值 & 10 个测试文件，176+ 条 CHECK，重复部分耗时节省约 60\% \\
...
\item \textbf{EnglishBallSelector 的交互设计}：3×3 网格在 80×80 像素内需要精确映射到 [-1.0, 1.0] 连续值，同时支持单击（选格子中心）和拖拽（连续微调）。最终方案是 mousePressEvent 定位最近格子中心并开始拖拽模式，mouseMoveEvent 计算当前鼠标相对网格中心的偏移量映射到加塞值。
...
\textbf{角色}：负责 Common 层（类型、常量、工具函数、DTO）、App 层（信号槽装配）、CMake 构建配置、全层测试体系搭建、架构护栏脚本、项目文档和报告撰写。
...
\item \textbf{提供预配置开发环境}：C++/Qt 项目环境搭建是最大痛点之一。不同平台（Windows/Mac/Linux）的 Qt 安装方式差异巨大（vcpkg 编译、aqtinstall 下载、官方安装器、包管理器），建议提供预配置的 Docker 镜像或 Dev Container 模板，让学生聚焦于代码开发而非环境调试。

\item \textbf{增加 MVVM 架构实战课时}：MVVM 在 Android/iOS 开发中常见，但在 C++/Qt 项目中较少被讲解。本项目的三轮架构重构如果能有一两次课堂讲解，可以避免学生走弯路。建议增加 1-2 课时的 MVVM 实战案例教学。
...
\item \textbf{引入 CI/CD 演示}：本项目虽在 CMake 中配置了架构护栏 custom target，但没有接入 GitHub Actions 自动运行。建议课程提供 CI/CD 配置模板（如 GitHub Actions + CMake + CTest），让学生理解持续集成的价值。

\item \textbf{大模型辅助开发的边界教育}：大模型在写重复性代码（测试、文档）时效率高，但在架构设计、参数调优、环境配置等需要实际经验的领域容易给出误导性建议。建议增加一节"如何正确使用 AI 编程助手"的内容，教学生识别 AI 方案的适用场景和风险点。

\item \textbf{报告模板提供}：LaTeX 排版对初学者有学习曲线。建议提供 LaTeX 报告模板（含封面、目录、图片占位、表格范例），让学生专注于内容填充。
...
\begin{itemize}
\item \textbf{完整的 MVVM 架构}：5 层编译目标，14 条跨层信号槽绑定，CMake 编译期强制边界约束
\item \textbf{加塞物理系统}：上塞/下塞/左塞/右塞，含角速度-线速度耦合碰撞、自旋衰减、滚动耦合
\item \textbf{完整斯诺克规则}：22 球标准摆球、交替击球、7 种犯规检测、自由球、输赢判定、彩球复位
\item \textbf{丰富的 UI 交互}：鼠标瞄准击球+球杆动画、库边预瞄辅助线、D 区白球放置、加塞面板、中英双语切换
\item \textbf{全层测试覆盖}：10 个测试可执行文件，176+ 条测试，覆盖 Common/Model/ViewModel/View 全部层级
...
\item \textbf{CI/CD 集成}：GitHub Actions 自动编译 + 运行测试 + 架构检查
